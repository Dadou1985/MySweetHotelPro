...
const AdminRegister = ({hide, user, userDB}) => {
    ...
    const handleCheckUserStatus = (email) => {
        let userStatus = checkUserStatus({email: email})
        console.log("#######", userStatus)
        if(userStatus === null) {
            return createUser({email: formValue.email, password: "password", username: formValue.username})
        }
    }
    ...
    const handleSubmit = async(event) => {
        ...
        //setFormValue("")
        await createUser({email: formValue.email, password: "password", username: formValue.username})
