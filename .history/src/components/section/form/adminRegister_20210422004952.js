...
const AdminRegister = ({hide, user, userDB}) => {
    ...
    const createUser = functions.h

    const handleSubmit = async(event) => {
        event.preventDefault()
        //setFormValue("")
        const newUser = await auth.createUserWithEmailAndPassword(formValue.email, "password");
        await newUser.user.updateProfile({displayName: formValue.username})
        //await resetPassword()
