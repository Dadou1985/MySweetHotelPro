...
import MaintenanceIcon from '../../../../assets/svg/repair.svg'
...
const Maintenance = () =>{
    ...
    const { mutate: addMaintenace } = useAdd(['hotels', userDB.hotelId, 'maintenance'])
    const { mutate: updateMaintenace } = useUpdate(['hotels', userDB.hotelId, 'maintenance'])
    const { mutate: deleteMaintenace } = useDelete(['hotels', userDB.hotelId, 'maintenance'])
    ...
    return(
        <div>
            <StyledBadge badgeContent={issueQty.length} color="secondary">
                <BadgeContent tooltipTitle={tooltipTitle} icon={MaintenanceIcon} handleShow={handleShow} />
            </StyledBadge>            
            <Modal show={list}
                size="xl"
                ...
                <Modal.Body>
                                            ...
                                            {info.map(flow =>(
                                                <tr key={flow.id}>
                                                    ...
                                                    <td>
                                                        <Switch
                                                            checked={flow.status}
                                                            onChange={() => updateMaintenace({ path: ['hotels', userDB.hotelId, 'maintenance', flow.id], data: dataStatus })}
                                                    ...
                                                    <td className="bg-dark"><Button variant="outline-danger" size="sm" onClick={()=> {
                                                        if(flow.img){
                                                            ...
                                                            return deleteMaintenace({ path: ['hotels', userDB.hotelId, 'maintenance', flow.id] })
                    ...
                    {footerState && <Modal.Footer>
                        <Button className='btn-msh-dark' onClick={(event) => {
                            ...
                            addMaintenace({ path: ['hotels', userDB.hotelId, 'maintenance'], data: newData })
